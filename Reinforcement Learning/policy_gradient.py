...
hidden_layer1 = tf.layers.dense(x, num_hidden, activation=tf.nn.elu, kernel_initializer=initializer)
...
for gradient, variable in gradients_and_vars:
    ...
    gradients_and_vars_feed.append((gradient_placeholder, variable))

train_op = optimizer.apply_gradients(gradients_and_vars_feed)
...
with tf.Session() as s:
    ...
    for iteration in range(num_iterations):
        ...
        for game in range(num_game_rounds):
            ...
            for step in range(max_game_steps):
                action_val, gradients_val = s.run([action, gradients], feed_dict={x: observations.reshape(1, num_inputs)})
    ...
    meta_graph_def = tf.train.export_meta_graph(filename='./models/policy_model.meta')
    saver.save(s, './models/policy_model')
...
with tf.Session() as s:
    ...
    new_saver = tf.train.import_meta_graph('./models/policy_model.meta')
    new_saver.restore(s, './models/policy_model')
